...
public class BlastFurnaceContainer extends BasicMachineContainer {
	...
	public BlastFurnaceContainer(InventoryPlayer player, BlastFurnaceTileEnt ent) {
		super(player, ent);
			...
			if(slot.getHasStack()){
				...
				if (!stack.isEmpty()){
					...
					if (id >=0 && id<=BlastFurnaceTileEnt.SLOT_UPGRADE){
						if (!this.mergeItemStack(stack1, BlastFurnaceTileEnt.SLOT_UPGRADE+1, MAXSLOTS, true)) {
